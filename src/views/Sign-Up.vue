...
<template>
	<div>
		...
		<a-card :bordered="false" class="card-signup header-solid h-full" :bodyStyle="{ paddingTop: 0 }">
			...
			<a-form id="components-form-demo-normal-login" ref="form" :model="form" class="login-form"
				@submit="handleSubmit">
				<a-form-item class="mb-10">
					<a-input v-model="form.name" :rules="[{ required: true, message: 'Please input your name!' }]"
						placeholder="Name" />
				</a-form-item>
				<a-form-item class="mb-10">
					<a-input v-model="form.email" :rules="[{ required: true, message: 'Please input your email!' }]"
						placeholder="email" />
				</a-form-item>
			...
			<p class="font-semibold text-muted text-center">Already have an account? <router-link to="/sign-in"
					class="font-bold text-dark">Sign In</router-link>
			</p>
		</a-card>
		<!-- / Sign Up Form -->
